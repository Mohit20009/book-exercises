...
summarise(to_sea, avg = mean(delayed_in_air,na.rm = T))
...
flights %>% 
  filter(dest=="SEA") %>% 
  summarise(avg_air_time= mean(air_time, na.rm = T), min_air_time = min(air_time, na.rm = T), max_air_time=max(air_time, na.rm = T))
