...
vehicle_model <- function(year_choice, make_choice){
   vehicle_year_make <- vehicles[vehicles$year == year_choice & vehicles$make == make_choice,]
   modell <- vehicle_year_make[vehicle_year_make$hwy == max(vehicle_year_make$hwy),"model"]
   return(modell)
}
